 ...
 * 
 * @author Michelle & Connor Phillips
 *
 */
public class BattleCharacter implements Comparable<BattleCharacter>{
    ...
    /**
     * Checks if two BattleCharacters are equal
    ...
    public boolean equals(Object obj)
    {
        if(!(obj instanceof BattleCharacter))
            return false;
        BattleCharacter x=(BattleCharacter) obj;
        if(x.getID()==this.ID)
            return true;
        return false;  
    }
    ...
    /**
     * Compares to Battle Characters
     * @param compObj the character to be compared
     * @return the valuse of this characters speed minus the other's
     */
    @Override
    public int compareTo(BattleCharacter compObj)
    {
        int send=this.stats[4]-compObj.getSpeed();
        if(send==0)
        {
            if(this.ID-compObj.getID()<0) 
                return 1;
            else if(this.ID-compObj.getID()>0)
                return -1;
        }
        return send;
    }
